...
export const CASE_STUDIES: Record<string, CaseStudy> = {
  ...
  easycab: {
    slug: 'easycab',
    stack: [
      'Python', 'Apache Kafka', 'PostgreSQL (central_db)', 'TCP/TLS sockets', 'JWT (HS256, 1h)',
      'HTTPS Registry', 'OpenWeatherMap API', 'Flask', 'Node.js / Express (dashboard)', 'REST audit API',
      'Pygame (central map)',
    ],
    diagram: {
      pipeline: ['EC_Customer', 'EC_Central', 'EC_DE · Taxis'],
      aside: ['Apache Kafka bus', 'PostgreSQL · central_db', 'TLS + JWT', 'EC_CTC · OpenWeather'],
    },
    en: {
      oneLiner: `An event-driven distributed ride-hailing system on Apache Kafka, where a central orchestrator matches autonomous taxis to customers on a toroidal grid, secured end to end with TLS and short-lived JWTs.`,
      problem: `EasyCab is the second practical assignment of a Distributed Systems course at the University of Alicante: build a fault-tolerant ride-hailing system out of independent processes that coordinate without shared memory. A central service must track and command a fleet of autonomous taxis, serve customers requesting trips on a shared 20×20 map, react to real-world conditions, and keep working when individual nodes drop — all over an asynchronous message bus, with authenticated and audited communication.`,
      role: `University Distributed Systems course project (Practice 2). Worked across the distributed architecture: the hub-and-spoke topology, the Kafka topic design, the central orchestrator and its PostgreSQL state with atomic taxi assignment, the security layer (Registry + TLS + JWT), the external weather integration, and the fault-tolerant reconnection logic. Academic scope: secrets are hardcoded and some auth helpers are stubs, so it demonstrates the distributed-systems concepts rather than production hardening.`,
      architecture: `A hub-and-spoke topology: a central orchestrator (EC_Central) coordinates autonomous taxis (EC_DE / Digital Engine) with their sensors (EC_S, over TCP sockets) and customers (EC_Customer) on a toroidal 20×20 map. The data plane is fully asynchronous over Apache Kafka — ride_requests/customer_responses between customers and the central, taxi_status (taxi→central, JWT-signed) and per-taxi taxi_orders_<id> plus a broadcast map_state (central→taxis). State is persisted in PostgreSQL (central_db) with atomic taxi assignment via SELECT ... FOR UPDATE SKIP LOCKED. An external module (EC_CTC) polls OpenWeatherMap; below 0 °C traffic flips to KO and the central recalls every taxi to base. Security adds two-step onboarding (HTTPS Registry → TLS socket) issuing HS256 JWTs (1h) verified on every message, with an audit_log surfaced through a REST API and a web dashboard.`,
      flow: [
        `A taxi onboards: it registers with the HTTPS Registry to obtain a certificate token, then authenticates to the central over a TLS socket and receives a short-lived JWT.`,
        `A customer publishes a trip to the ride_requests Kafka topic; the central consumes it and atomically assigns a free taxi using FOR UPDATE SKIP LOCKED in PostgreSQL.`,
        `The central commands the assigned taxi on its taxi_orders_<id> topic and broadcasts the world on map_state; the taxi (EC_DE) drives toward pickup and destination on the 20×20 toroidal grid.`,
        `Each taxi streams JWT-signed taxi_status updates; sensors (EC_S) report OK/KO over a TCP socket so the taxi stops on incidents.`,
        `EC_CTC polls OpenWeatherMap; if the temperature drops below 0 °C it sets traffic to KO and the central orders every taxi back to base.`,
        `If a node drops, a 10s reconnection window lets it resume the in-flight service (resume_service); if it expires, the central marks the incident and reassigns the customer.`,
        `Every security-relevant event is written to audit_log and surfaced through a REST API and a live web dashboard.`,
      ],
      decisions: [
        { decision: `Apache Kafka pub/sub as the data plane (per-taxi command topics + a broadcast map_state topic)`, why: `Decouples producers from consumers so taxis, customers and the central never call each other directly; per-taxi topics give targeted commands while one broadcast topic keeps the whole fleet's world view consistent.` },
        { decision: `Atomic taxi assignment with SELECT ... FOR UPDATE SKIP LOCKED in PostgreSQL`, why: `Many ride requests can be matched concurrently; row-level locking with SKIP LOCKED guarantees no two customers get the same taxi without serializing the entire assignment path.` },
        { decision: `Two-step security: an HTTPS Registry for onboarding, then TLS sockets and short-lived HS256 JWTs verified per message`, why: `Separates identity bootstrap from the live session and protects both transport (TLS) and message integrity (JWT), so a process can't spoof status updates or commands on the bus.` },
        { decision: `An external weather service (EC_CTC → OpenWeatherMap) wired into control logic`, why: `Shows an outside dependency changing system behavior: a real condition (sub-zero temperature) propagates through the system and recalls the whole fleet, not just a cosmetic readout.` },
        { decision: `A bounded 10s reconnection window with in-flight service restoration (resume_service)`, why: `In a distributed system nodes drop; bounding the reconnection window lets a recovered node resume its current trip instead of losing it, and cleanly reassigns the customer if recovery fails.` },
      ],
      challenges: [
        `Coordinating independent processes with no shared memory: solved by modeling every interaction as a Kafka message on well-defined topics, keeping the central, taxis and customers decoupled and observable.`,
        `Avoiding double-assignment of a taxi under concurrent requests: solved with PostgreSQL row locking (FOR UPDATE SKIP LOCKED) so assignment is atomic.`,
        `Securing a message bus where any process could publish: addressed with TLS onboarding, per-message JWT verification and an audit trail, so messages are authenticated and traceable.`,
        `Surviving partial failures: handled with a bounded reconnection window that restores in-flight service or reassigns the customer, keeping the system live when nodes drop.`,
      ],
      results: [
        `Working distributed ride-hailing system: a central orchestrator matching autonomous taxis to customers on a 20×20 toroidal map, fully decoupled over Apache Kafka.`,
        `End-to-end security: HTTPS Registry onboarding, TLS-authenticated sockets, and HS256 JWTs (1h) verified on every message, with a full audit_log surfaced via a REST API and web dashboard.`,
        `External weather integration (OpenWeatherMap) wired into control logic: sub-zero temperatures recall the entire fleet to base.`,
        `Fault tolerance via a 10s reconnection window with in-flight service restoration and customer reassignment. Academic project: secrets (JWT key, weather API key) are hardcoded and some auth helpers are stubs, so it demonstrates the distributed-systems concepts rather than production hardening.`,
      ],
    },
    es: {
      oneLiner: `Un sistema distribuido de transporte tipo Uber orientado a eventos sobre Apache Kafka, donde una central orquesta taxis autónomos y clientes en un mapa toroidal, con seguridad de extremo a extremo mediante TLS y JWT de corta duración.`,
      problem: `EasyCab es la segunda práctica de la asignatura de Sistemas Distribuidos de la Universidad de Alicante: construir un sistema de transporte tolerante a fallos a partir de procesos independientes que se coordinan sin memoria compartida. Una central debe seguir y mandar una flota de taxis autónomos, atender a clientes que piden viajes en un mapa 20×20 compartido, reaccionar a condiciones del mundo real y seguir funcionando cuando caen nodos individuales — todo sobre un bus de mensajes asíncrono, con comunicación autenticada y auditada.`,
      role: `Proyecto universitario de Sistemas Distribuidos (Práctica 2). Trabajo sobre la arquitectura distribuida: la topología hub-and-spoke, el diseño de topics de Kafka, la central orquestadora y su estado en PostgreSQL con asignación atómica de taxis, la capa de seguridad (Registry + TLS + JWT), la integración con la API del clima y la lógica de reconexión tolerante a fallos. Alcance académico: los secretos están hardcodeados y algunos ayudantes de auth son stubs, así que demuestra los conceptos de sistemas distribuidos más que un endurecimiento de producción.`,
      architecture: `Una topología hub-and-spoke: una central (EC_Central) coordina taxis autónomos (EC_DE / Digital Engine) con sus sensores (EC_S, por sockets TCP) y clientes (EC_Customer) en un mapa toroidal 20×20. El plano de datos es totalmente asíncrono sobre Apache Kafka — ride_requests/customer_responses entre clientes y central, taxi_status (taxi→central, firmado con JWT) y, por taxi, taxi_orders_<id> más un map_state de difusión (central→taxis). El estado se persiste en PostgreSQL (central_db) con asignación atómica de taxis mediante SELECT ... FOR UPDATE SKIP LOCKED. Un módulo externo (EC_CTC) consulta OpenWeatherMap; por debajo de 0 °C el tráfico pasa a KO y la central ordena a todos los taxis volver a base. La seguridad añade un alta en dos pasos (Registry HTTPS → socket TLS) que emite JWT HS256 (1h) verificados en cada mensaje, con un audit_log expuesto por una API REST y un panel web.`,
      flow: [
        `Un taxi se da de alta: se registra en el Registry HTTPS para obtener un token de certificado, luego se autentica con la central por un socket TLS y recibe un JWT de corta duración.`,
        `Un cliente publica un viaje en el topic ride_requests de Kafka; la central lo consume y asigna atómicamente un taxi libre con FOR UPDATE SKIP LOCKED en PostgreSQL.`,
        `La central manda al taxi asignado por su topic taxi_orders_<id> y difunde el mundo por map_state; el taxi (EC_DE) conduce hacia la recogida y el destino en el mapa toroidal 20×20.`,
        `Cada taxi emite actualizaciones taxi_status firmadas con JWT; los sensores (EC_S) reportan OK/KO por un socket TCP para que el taxi se detenga ante incidencias.`,
        `EC_CTC consulta OpenWeatherMap; si la temperatura baja de 0 °C pone el tráfico en KO y la central ordena a todos los taxis volver a base.`,
        `Si un nodo cae, una ventana de reconexión de 10s le permite retomar el servicio en curso (resume_service); si expira, la central marca la incidencia y reasigna al cliente.`,
        `Cada evento relevante de seguridad se escribe en audit_log y se expone por una API REST y un panel web en vivo.`,
      ],
      decisions: [
        { decision: `Pub/sub con Apache Kafka como plano de datos (topics de comando por taxi + un topic map_state de difusión)`, why: `Desacopla productores de consumidores para que taxis, clientes y central nunca se llamen directamente; los topics por taxi dan comandos dirigidos mientras un único topic de difusión mantiene consistente la visión del mundo de toda la flota.` },
        { decision: `Asignación atómica de taxis con SELECT ... FOR UPDATE SKIP LOCKED en PostgreSQL`, why: `Muchas peticiones pueden emparejarse a la vez; el bloqueo a nivel de fila con SKIP LOCKED garantiza que dos clientes nunca reciban el mismo taxi sin serializar toda la ruta de asignación.` },
        { decision: `Seguridad en dos pasos: un Registry HTTPS para el alta, luego sockets TLS y JWT HS256 de corta duración verificados por mensaje`, why: `Separa el arranque de identidad de la sesión en vivo y protege tanto el transporte (TLS) como la integridad de mensaje (JWT), de modo que un proceso no pueda falsificar estados o comandos en el bus.` },
        { decision: `Un servicio de clima externo (EC_CTC → OpenWeatherMap) conectado a la lógica de control`, why: `Muestra una dependencia externa cambiando el comportamiento del sistema: una condición real (temperatura bajo cero) se propaga por el sistema y recoge a toda la flota, no es solo un dato cosmético.` },
        { decision: `Una ventana de reconexión acotada de 10s con restauración del servicio en curso (resume_service)`, why: `En un sistema distribuido los nodos caen; acotar la ventana de reconexión permite a un nodo recuperado retomar su viaje actual en vez de perderlo, y reasigna limpiamente al cliente si la recuperación falla.` },
      ],
      challenges: [
        `Coordinar procesos independientes sin memoria compartida: resuelto modelando cada interacción como un mensaje de Kafka en topics bien definidos, manteniendo central, taxis y clientes desacoplados y observables.`,
        `Evitar la doble asignación de un taxi ante peticiones concurrentes: resuelto con bloqueo de fila en PostgreSQL (FOR UPDATE SKIP LOCKED) para que la asignación sea atómica.`,
        `Asegurar un bus de mensajes donde cualquier proceso podría publicar: abordado con alta por TLS, verificación de JWT por mensaje y un registro de auditoría, para que los mensajes sean autenticados y trazables.`,
        `Sobrevivir a fallos parciales: gestionado con una ventana de reconexión acotada que restaura el servicio en curso o reasigna al cliente, manteniendo el sistema vivo cuando caen nodos.`,
      ],
      results: [
        `Sistema distribuido de transporte funcional: una central que empareja taxis autónomos con clientes en un mapa toroidal 20×20, totalmente desacoplado sobre Apache Kafka.`,
        `Seguridad de extremo a extremo: alta por Registry HTTPS, sockets autenticados con TLS y JWT HS256 (1h) verificados en cada mensaje, con un audit_log completo expuesto por una API REST y un panel web.`,
        `Integración con clima externo (OpenWeatherMap) conectada a la lógica de control: las temperaturas bajo cero recogen a toda la flota a base.`,
        `Tolerancia a fallos mediante una ventana de reconexión de 10s con restauración del servicio en curso y reasignación de cliente. Proyecto académico: los secretos (clave JWT, API key del clima) están hardcodeados y algunos ayudantes de auth son stubs, así que demuestra los conceptos de sistemas distribuidos más que un endurecimiento de producción.`,
      ],
    },
  },
}
